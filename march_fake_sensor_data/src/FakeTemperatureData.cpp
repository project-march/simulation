...
double calculateArTemperature(std::vector<int> temperatures,
                              std::vector<float> ar_values) {
  double res = 0;
 ...
 */
void temperatureConfigCallback(
    march_fake_sensor_data::TemperaturesConfig &config, uint32_t level) {
...
void publishTemperature(const ros::Publisher &temperature_pub) {
  ...
  double current_temperature =
      calculateArTemperature(latest_temperatures, ar_values);
...
std::string createTopicName(const char *base, const char *name) {
  ...
  const int kArraySize = static_cast<const int>(
      strlen(base) + strlen(slash) + strlen(name) + extra_buffer_size);
  ...
  if (error_code < 0 || error_code > kArraySize) {
    ROS_ERROR("Error creating topic %s (error code %d)", full_topic,
              error_code);
  }
  return full_topic;
}
...
int main(int argc, char **argv) {
  ...
  while (!n.hasParam("/march/joint_names")) {
    ...
    if (count > 10) {
      ...
      throw std::runtime_error(
          "Failed to read the joint_names from the parameter server.");
    }
  }
  ...
  for (std::string sensor_name : sensor_names) {
    ros::Publisher temperature_pub = n.advertise<sensor_msgs::Temperature>(
        createTopicName("/march/temperature", sensor_name.c_str()), 1000);
    temperature_publishers.push_back(temperature_pub);
  }

  // Make the temperature values dynamic reconfigurable
  dynamic_reconfigure::Server<march_fake_sensor_data::TemperaturesConfig>
      server;
  dynamic_reconfigure::Server<
      march_fake_sensor_data::TemperaturesConfig>::CallbackType f;
